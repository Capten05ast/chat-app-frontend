...
function SenderAvatar({ sender, size = 32 }) {
  const name = sender?.fullName?.firstName || sender?.firstName || "?";
...
function GroupMessage({ msg, currentUser, groupId, onDeleted, showAvatar, showName, allMembersSeen }) {
  // ✅ FIX: Robust ID comparison (handles both objects and strings)
  ...
  const handleDelete = async (e) => {
    ...
    setDeleting(true);
    try { 
      // Ensure we use the string ID for the API call
      await axios.delete(`/group-messages/${groupId?.toString()}/${msg._id?.toString()}`); 
      onDeleted?.(msg._id); 
    }
    catch (err) { 
      console.log(err); 
          ...
          <div className={`flex flex-col max-w-[78%] sm:max-w-[62%] ${isMe ? "items-end" : "items-start"}`}>
            {showName && !isMe && (
              <span className="text-[12px] font-bold text-gray-600 mb-1 ml-1">
                {msg.sender?.fullName?.firstName || msg.sender?.firstName || "Unknown"}
              </span>
            )}
